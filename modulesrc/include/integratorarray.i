...
// ----------------------------------------------------------------------
// List of mesh integrators.
%typemap(in) (pylith::feassemble::IntegratorPointwise* integratorArray[],
	      const int numIntegrators)
{
  // Check to make sure input is a list.
  if (PyList_Check($input)) {
    const int size = PyList_Size($input);
    $2 = size;
    $1 = (size > 0) ? new pylith::feassemble::IntegratorPointwise*[size] : 0;
    for (int i = 0; i < size; i++) {
      PyObject* s = PyList_GetItem($input,i);
      pylith::feassemble::IntegratorPointwise* integrator = 0;
      int err = SWIG_ConvertPtr(s, (void**) &integrator,
				$descriptor(pylith::feassemble::IntegratorPointwise*),
				0);
      if (SWIG_IsOK(err))
	$1[i] = (pylith::feassemble::IntegratorPointwise*) integrator;
      else {
	PyErr_SetString(PyExc_TypeError, "List must contain integrators.");
	delete[] $1;
	return NULL;
...
// This cleans up the array we malloc'd before the function call
%typemap(freearg) (pylith::feassemble::IntegratorPointwise* integratorArray[],
		   const int numIntegrators) {
  delete[] $1;
}
