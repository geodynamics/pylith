...
The inversion results will be written to `output/step06_inversion-results.txt`.

The order of the impulses and stations in the output depends on the partitioning among processes.
...
class InvertSlipApp:
    ...
    def main(self):
        ...
        results = self.invert()
        self.write_results(args.filename_output, results)


    def get_fault_impulses(self, filename):
        """Get coordinates, amplitude, and order of impulses. Fault points are in a random order

        :param filename: Name of HDF5 file with fault data from PyLith Green's function simulation (Step 5).
        """
        with h5py.File(filename, "r") as h5:
            self.faultpts = numpy.array(h5['geometry/vertices'])
            strike_slip = h5['vertex_fields/slip'][:,:,1]
            dip_slip = h5['vertex_fields/slip'][:,:,2]
            self.impulse_slip = numpy.column_stack((strike_slip,dip_slip))
    ...
    def get_station_responses(self, filename):
        ...
        """
        with h5py.File(filename, "r") as h5:
            self.station_responses = numpy.array(h5['vertex_fields/displacement'])
    ...
    def invert(self):
        ...
        results = numpy.zeros((nfaultpts, 1 + 2*ninversions))
        results[:,0] = range(nfaultpts)
        ...
        for i, penalty in enumerate(self.penalties):
            ...
            # save slip solution to columns of the output
            results[:, 2*i+1] = slip_soln[0:nfaultpts]
            results[:, 2*i+2] = slip_soln[nfaultpts:]
    ...
    def write_results(self, filename, results):
        ...
        for penalty in self.penalties:
            header += f", penalty={penalty}_ss, penalty={penalty}_ds"
        header += "\n"
