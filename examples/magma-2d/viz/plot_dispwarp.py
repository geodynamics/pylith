...
import os
OUTPUT_DIR = os.path.join(os.environ["HOME"], "src", "pylith", "examples", "magma-2d", "output")
"""

DEFAULTS = {
    "OUTPUT_DIR": "output",
    "SIM": "step01_inflation",
    "WARP_SCALE": 1.0e+3,
    "FIELD": "pressure",
...
def visualize(parameters):
    ...
    if parameters.field_component.lower() == "magnitude":
        fieldName = parameters.field.capitalize()
        units = "(Pa)" if parameters.field == "pressure" else "(m)"
        colorbar.Title = f"{fieldName} {units}"
