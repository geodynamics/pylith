...
\begin{itemize}
\item Added \commandline{-{}-version} command line argument to
  display version information for PyLith and its dependencies.
\item Improved information displayed with the \commandline{-{}-help}
  command line argument.
\item Added \commandline{-{}-include-citations} command line
  argument to display publications to cite when publishing results
  from computations using PyLith. General PyLith vreferences are also
  displayed with the \commandline{-{}-version} command line
  argument.
\item Allow use of NetCDF versions greater than 4.1.3. Switch from
  using C++ API to C API.
\item Fixed bug in Pythia associated with validation of parameters
  being done before help could be displayed.
