...
\warning{The binary distribution contains PyLith and all of its
  ...
  conflicts. On Darwin systems PyLith is built against the system
  Python, so \filename{/bin} and \filename{/usr/bin} should be at the
  beginning of your PATH before sourcing \filename{setup.sh}.}
...
# Allow X connections
$$ xhost +local:root
# Use a shell variable to hold the name of the X11 socket.
