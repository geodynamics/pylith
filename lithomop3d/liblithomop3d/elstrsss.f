...
c
c
      subroutine elstrsss(x,b,d,dx,tfault,nsd,ndof,numnp,neq,numfn,
     & numslp,
...
c
      ...
      integer ndmatsz,numelt,nconsz,numat,netypes,nskdim,numrot
      integer idebug,idout,kto,kw,ierr
...
c...  external routines
c
      include "elstrsss_ext.inc"
      external getshape,bmatrix
c
c...  local variables
c
      integer imat,matmodel,nmatel
c
cdebug      write(6,*) "Hello from elstrsss_f!"
c
      call dscal(neq,zero,b,ione)
      matgpt=1
c
...
c
      do imat=1,numat
        ...
        if(matmodel.eq.1) then
          call elstrsscmp(x,b,d,dx,tfault,nsd,ndof,numnp,neq,numfn,
     &     numslp,
     &     state,dmat,nstr,nddmat,nstatesz,ndmatsz,
     &     ien,lm,lmx,lmf,infiel,numelt,nconsz,
...
c
c version
c $Id: elstrsss.f,v 1.1 2004/05/20 21:08:57 willic3 Exp $
c
