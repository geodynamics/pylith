...
def addFileNameTags(filename):
    ...
    line = f.readline()
    while line:
        if not (removedefines and line.startswith('#define ')):
            g.write(line)
...
def processDir(tagfiles, dirname, names):
    ...
    gsfx = ['.py',
            ...
            '.tex', 'makefile', '.bib']
    hsfx = ['.html']
    bsfx = ['.py.html', '.c.html', '.F.html', '.h.html', '.tex.html',
            '.cxx.html', '.hh.html', 'makefile.html', '.gcov.html']
    for l in names:
        if endsWithSuffix(l, gsfx):
            ...
            newls.append(l)
    if newls:
        createTags(etagfile, ctagfile, dirname, newls)
    ...
    for glist in lists:
        ...
        for file in gfiles:
            if not (file.endswith('pyc') or file.endswith('/SCCS') or file.endswith('~')):
                files.append(file)
    if files:
        createTags(etagfile, ctagfile, dirname, files)
    return
