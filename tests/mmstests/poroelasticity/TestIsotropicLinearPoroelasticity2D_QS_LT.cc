...
protected:
    ...
    void _setExactSolution(void) {
        ...
        err = DMGetLabel(_solution->dmMesh(), "material-id", &label);CPPUNIT_ASSERT(!err);
        ...
        err = PetscWeakFormSetIndexResidual(wf, label, 24, 0, 0, 1, pylith::fekernels::IsotropicLinearPoroelasticityPlaneStrain::f0_mms_ql_u, 1, NULL);PYLITH_CHECK_ERROR(err);
        err = PetscWeakFormSetIndexResidual(wf, label, 24, 1, 0, 1, pylith::fekernels::IsotropicLinearPoroelasticityPlaneStrain::f0_mms_ql_p, 1, NULL);PYLITH_CHECK_ERROR(err);
    } // _setExactSolution
