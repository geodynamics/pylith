...
def getVars(fileName):
    ...
    return (timeYears, sxx, ezz, dispz)
...
for simNum in range(numSims):
    ...
    (time, sxx, ezz, dispz) = getVars(h5File)
    jacobianDiff[simNum] = scanLogfile(logFile)
    a[0][0].plot(time, sxx, lineDefs[simNum], label="dt={0}".format(stepSizesStr[simNum]))
    a[1][0].plot(time, ezz, lineDefs[simNum], label="dt={0}".format(stepSizesStr[simNum]))
    a[0][1].plot(time, dispz, lineDefs[simNum], label="dt={0}".format(stepSizesStr[simNum]))
