...
bqSuffixes = ['b1_q1_', 'b2_q2_', 'b3_q3_']
meshPrefix = 'meshes/mesh_cylinder_'
outputPrefix = 'cylinder_pres_elastic_compare_'
cellTypes = ['hex', 'tet']
...
for cellType in cellTypes:
    for bqSuff in bqSuffixes:
        ...
        dispAnlTiled = numpy.repeat(dispAnl.reshape(1, nverts, 3), nsteps, axis=0)
        ...
        dispAnlOut = h5Out.create_dataset('vertex_fields/displacement_anl',
                                          data=dispAnlTiled.reshape(nsteps, nverts, 3), maxshape=(None, nverts, 3))
        dispAnlOut.attrs['vector_field_type'] = 'vector'.encode('ascii')
        dispNumOut = h5Out.create_dataset('vertex_fields/displacement_num',
                                          data=dispNum.reshape(nsteps, nverts, 3), maxshape=(None, nverts, 3))
        ...
        dispAnlMinusNum = h5Out.create_dataset('vertex_fields/displacement_anl_minus_num',
                                               data=dispDiff.reshape(nsteps, nverts, 3), maxshape=(None, nverts, 3))
