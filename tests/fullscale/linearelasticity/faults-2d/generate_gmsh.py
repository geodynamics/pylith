...
class App(GenerateMesh):
    ...
    def mark(self):
        ...
        face_groups = (
            ...
            BoundaryGroup(name="boundary_xpos", tag=11, dim=1, entities=[self.l_xpos0, self.l_xpos1]),
            ...
            BoundaryGroup(name="fault_xmid", tag=20, dim=1, entities=[self.l_fault0, self.l_fault1]),
            BoundaryGroup(name="fault_xneg", tag=21, dim=1, entities=[self.l_split0]),
        )
        for group in face_groups:
            group.create_physical_group()
        patch_groups = (
            BoundaryGroup(name="patch_xmid", tag=30, dim=2, entities=[self.s_xmid]),
        )
