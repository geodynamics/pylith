...
class AnalyticalSoln(object):
    ...
    def __init__(self):
        self.fields = {
            ...
            "initial_amplitude": {
                ...
                "bc_press_xneg": self.pressure_zero,
            },
            "normal_dir": {
    ...
    def trace_strain(self, locs):
        ...
        ev = -1.0 / (p_lambda + 2.0 * p_mu) * (1.0 - p_alpha) * fx * (x - x_min)
    ...
    def strain(self, locs):
        ...
        exx = -1.0 / (p_lambda + 2.0 * p_mu) * (1.0 - p_alpha) * fx * (x - x_min)
        strain[0, :, 0] = exx
        strain[0, :, 1] = 0.0
        strain[0, :, 2] = 0.0
        strain[0, :, 3] = 0.0
    ...
    def stress(self, locs):
        ...
        p_alpha = p_biot_coefficient
        sxx = -fx * (x - x_min)
        syy = szz = -p_lambda / (p_lambda + 2 * p_mu) * fx * (x - x_min) - 2 * p_mu / (
            p_lambda + 2 * p_mu
        ) * p_alpha * fx * (x - x_min)
