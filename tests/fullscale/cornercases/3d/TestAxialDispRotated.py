...
# ----------------------------------------------------------------------
#
# @file tests/fullscale/cornercases/3d/TestAxialDispRotated.py
#
# @brief Test suite for testing pylith with 2-D axial extension in rotated coordinates.
...
from axialdisp_rotated_gendb import GenerateDB


# ----------------------------------------------------------------------------------------------------------------------
class TestCase(FullTestCase):
    """
    Test suite for testing PyLith with axial extension in the x-direction for a rotated mesh.
    """
...
class TestHex(TestCase, meshes.Hex):
    NAME = "axialdisp_rotated_hex"

    def setUp(self):
        TestCase.setUp(self)
        TestCase.run_pylith(
            self, self.NAME, ["axialdisp_rotated.cfg", "axialdisp_rotated_hex.cfg"])
        return

# ----------------------------------------------------------------------------------------------------------------------
class TestTet(TestCase, meshes.Tet):
    NAME = "axialdisp_rotated_tet"

    def setUp(self):
        TestCase.setUp(self)
        TestCase.run_pylith(
            self, self.NAME, ["axialdisp_rotated.cfg", "axialdisp_rotated_tet.cfg"])
        return
