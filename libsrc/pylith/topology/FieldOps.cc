...
{ // createFE
    ...
    if (isSimplex) {
      err = PetscDTGaussJacobiQuadrature(dim, 1, quadOrder, -1.0, 1.0, &quadrature); PYLITH_CHECK_ERROR(err);
    } else {
      err = PetscDTGaussTensorQuadrature(dim, 1, quadOrder, -1.0, 1.0, &quadrature); PYLITH_CHECK_ERROR(err);
    }
