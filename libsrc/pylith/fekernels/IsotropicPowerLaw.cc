...
pylith::fekernels::IsotropicPowerLawPlaneStrain::g1v(const PylithInt dim,
    ...
    const PylithInt i_bulkModulus = numA-7;
    const PylithInt i_powerLawReferenceStrainRate = numA-6;
    const PylithInt i_powerLawReferenceStress = numA-5;
    const PylithInt i_powerLawExponent = numA-4;
    const PylithInt i_powerLawAlpha = numA-3;
    ...
    const PylithInt aOffDev[7] = {aOff[i_shearModulus], aOff[i_powerLawReferenceStrainRate], aOff[i_powerLawReferenceStress],
								  aOff[i_powerLawExponent], aOff[i_powerLawAlpha], aOff[i_viscousStrain], aOff[i_stress]};
...
pylith::fekernels::IsotropicPowerLawPlaneStrain::g1v_refstate(const PylithInt dim,
    ...
    // Incoming auxiliary fields.
    const PylithInt i_rstress = numA-10;
    const PylithInt i_rstrain = numA-9;
    const PylithInt i_shearModulus = numA-8;
    const PylithInt i_bulkModulus = numA-7;
    const PylithInt i_powerLawReferenceStrainRate = numA-6;
    const PylithInt i_powerLawReferenceStress = numA-5;
    ...
    const PylithInt numADev = 9; // Number passed to deviatoric stress kernel.
    const PylithInt aOffDev[9] = {aOff[i_rstress], aOff[i_rstrain], aOff[i_shearModulus], aOff[i_powerLawReferenceStrainRate],
								  aOff[i_powerLawReferenceStress], aOff[i_powerLawExponent], aOff[i_powerLawAlpha],
								  aOff[i_viscousStrain], aOff[i_stress]};
...
pylith::fekernels::IsotropicPowerLawPlaneStrain::Jg3vu(const PylithInt dim,
    ...
    /* j(f,g,df,dg) = C(f,df,g,dg)
	   ...
	   1:  j0001 = C1112 = 0
	   2:  j0010 = C1211 = 0
	   3:  j0011 = C1212 = -1/(2*(alpha**2*deltaT*gammaFTau*s12**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s12*s12T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   4:  j0100 = C1121 = 0
	   5:  j0101 = C1122 = bulkModulus + 1/(3*(alpha**2*deltaT*gammaFTau*s11*s22*(n - 1)/(2*j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau*s11*s22T*(-alpha + 1)*(n - 1)/(2*j2FTau*j2FTplusDt)))
	   6:  j0110 = C1221 = -1/(2*(alpha**2*deltaT*gammaFTau*s12**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s12*s12T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   7:  j0111 = C1222 = 0
	   8:  j1000 = C2111 = 0
	   9:  j1001 = C2112 = -1/(2*(alpha**2*deltaT*gammaFTau*s12**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s12*s12T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   10:  j1010 = C2211 = bulkModulus + 1/(3*(alpha**2*deltaT*gammaFTau*s11*s22*(n - 1)/(2*j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau*s11T*s22*(-alpha + 1)*(n - 1)/(2*j2FTau*j2FTplusDt)))
	   11:  j1011 = C2212 = 0
	   12:  j1100 = C2121 = -1/(2*(alpha**2*deltaT*gammaFTau*s12**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s12*s12T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   13:  j1101 = C2122 = 0
	   14:  j1110 = C2221 = 0
	   15:  j1111 = C2222 = bulkModulus - 2/(3*(alpha**2*deltaT*gammaFTau*s22**2*(n - 1)/(2*j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s22*s22T*(-alpha + 1)*(n - 1)/(2*j2FTau*j2FTplusDt) + 1/(2*mu)))
	*/
...
pylith::fekernels::IsotropicPowerLawPlaneStrain::deviatoricStress(const PylithInt dim,
    ...
    const PylithInt i_powerLawAlpha = 4;
    const PylithInt i_viscousStrain = 5;
    const PylithInt i_stress = 6;
...
pylith::fekernels::IsotropicPowerLawPlaneStrain::deviatoricStress_refstate(const PylithInt dim,
    ...
    const PylithInt i_powerLawAlpha = 6;
    const PylithInt i_viscousStrain = 7;
    const PylithInt i_stress = 8;
...
pylith::fekernels::IsotropicPowerLawPlaneStrain::deviatoricStress4(const PylithInt dim,
    ...
    const PylithScalar devStrainTpdt[4] = {strainTpdt[0] - meanStrainTpdt,
										   strainTpdt[1] - meanStrainTpdt,
										   -meanStrainTpdt,
										   strainTpdt[2]};
	const PylithScalar strainPPTpdt[4] = {devStrainTpdt[0] - visStrainT[0],
...
// ---------------------------------------------------------------------------------------------------------------------
// Calculate stress for 2-D plane strain isotropic power-law viscoelastic material WITHOUT a reference stress and strain.
void
...
// ---------------------------------------------------------------------------------------------------------------------
// Calculate stress for 2-D plane strain isotropic linear power-law viscoelastic material WITH a reference stress/strain.
void
...
pylith::fekernels::IsotropicPowerLaw3D::Jg3vu(const PylithInt dim,
    ...
    /* j(f,g,df,dg) = C(f,df,g,dg)
	   ...
	   2:  j0002 = C1113 = 0
	   3:  j0010 = C1211 = 0
	   4:  j0011 = C1212 = -1/(2*(alpha**2*deltaT*gammaFTau*s12**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s12*s12T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   5:  j0012 = C1213 = 0
	   6:  j0020 = C1311 = 0
	   7:  j0021 = C1312 = 0
	   8:  j0022 = C1313 = -1/(2*(alpha**2*deltaT*gammaFTau*s13**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s13*s13T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   9:  j0100 = C1121 = 0
	   10:  j0101 = C1122 = bulkModulus + 1/(3*(alpha**2*deltaT*gammaFTau*s11*s22*(n - 1)/(2*j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau*s11*s22T*(-alpha + 1)*(n - 1)/(2*j2FTau*j2FTplusDt)))
	   11:  j0102 = C1123 = 0
	   12:  j0110 = C1221 = -1/(2*(alpha**2*deltaT*gammaFTau*s12**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s12*s12T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   13:  j0111 = C1222 = 0
	   14:  j0112 = C1223 = 0
	   15:  j0120 = C1321 = 0
	   16:  j0121 = C1322 = 0
	   ...
	   18:  j0200 = C1131 = 0
	   19:  j0201 = C1132 = 0
	   20:  j0202 = C1133 = bulkModulus + 1/(3*(alpha**2*deltaT*gammaFTau*s11*s33*(n - 1)/(2*j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau*s11*s33T*(-alpha + 1)*(n - 1)/(2*j2FTau*j2FTplusDt)))
	   21:  j0210 = C1231 = 0
	   22:  j0211 = C1232 = 0
	   23:  j0212 = C1233 = 0
	   24:  j0220 = C1331 = -1/(2*(alpha**2*deltaT*gammaFTau*s13**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s13*s13T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   25:  j0221 = C1332 = 0
	   26:  j0222 = C1333 = 0
	   27:  j1000 = C2111 = 0
	   28:  j1001 = C2112 = -1/(2*(alpha**2*deltaT*gammaFTau*s12**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s12*s12T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   29:  j1002 = C2113 = 0
	   30:  j1010 = C2211 = bulkModulus + 1/(3*(alpha**2*deltaT*gammaFTau*s11*s22*(n - 1)/(2*j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau*s11T*s22*(-alpha + 1)*(n - 1)/(2*j2FTau*j2FTplusDt)))
	   31:  j1011 = C2212 = 0
	   32:  j1012 = C2213 = 0
	   33:  j1020 = C2311 = 0
	   34:  j1021 = C2312 = 0
	   35:  j1022 = C2313 = 0
	   36:  j1100 = C2121 = -1/(2*(alpha**2*deltaT*gammaFTau*s12**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s12*s12T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   37:  j1101 = C2122 = 0
	   38:  j1102 = C2123 = 0
	   39:  j1110 = C2221 = 0
	   40:  j1111 = C2222 = bulkModulus - 2/(3*(alpha**2*deltaT*gammaFTau*s22**2*(n - 1)/(2*j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s22*s22T*(-alpha + 1)*(n - 1)/(2*j2FTau*j2FTplusDt) + 1/(2*mu)))
	   41:  j1112 = C2223 = 0
	   42:  j1120 = C2321 = 0
	   43:  j1121 = C2322 = 0
	   44:  j1122 = C2323 = -1/(2*(alpha**2*deltaT*gammaFTau*s23**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s23*s23T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   45:  j1200 = C2131 = 0
	   46:  j1201 = C2132 = 0
	   47:  j1202 = C2133 = 0
	   48:  j1210 = C2231 = 0
	   49:  j1211 = C2232 = 0
	   50:  j1212 = C2233 = bulkModulus + 1/(3*(alpha**2*deltaT*gammaFTau*s22*s33*(n - 1)/(2*j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau*s22*s33T*(-alpha + 1)*(n - 1)/(2*j2FTau*j2FTplusDt)))
	   51:  j1220 = C2331 = 0
	   52:  j1221 = C2332 = -1/(2*(alpha**2*deltaT*gammaFTau*s23**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s23*s23T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   53:  j1222 = C2333 = 0
	   54:  j2000 = C3111 = 0
	   55:  j2001 = C3112 = 0
	   56:  j2002 = C3113 = -1/(2*(alpha**2*deltaT*gammaFTau*s13**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s13*s13T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   57:  j2010 = C3211 = 0
	   58:  j2011 = C3212 = 0
	   59:  j2012 = C3213 = 0
	   60:  j2020 = C3311 = bulkModulus + 1/(3*(alpha**2*deltaT*gammaFTau*s11*s33*(n - 1)/(2*j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau*s11T*s33*(-alpha + 1)*(n - 1)/(2*j2FTau*j2FTplusDt)))
	   61:  j2021 = C3312 = 0
	   62:  j2022 = C3313 = 0
	   63:  j2100 = C3121 = 0
	   64:  j2101 = C3122 = 0
	   ...
	   66:  j2110 = C3221 = 0
	   67:  j2111 = C3222 = 0
	   68:  j2112 = C3223 = -1/(2*(alpha**2*deltaT*gammaFTau*s23**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s23*s23T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   69:  j2120 = C3321 = 0
	   70:  j2121 = C3322 = bulkModulus + 1/(3*(alpha**2*deltaT*gammaFTau*s22*s33*(n - 1)/(2*j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau*s22T*s33*(-alpha + 1)*(n - 1)/(2*j2FTau*j2FTplusDt)))
	   71:  j2122 = C3323 = 0
	   72:  j2200 = C3131 = -1/(2*(alpha**2*deltaT*gammaFTau*s13**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s13*s13T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   73:  j2201 = C3132 = 0
	   74:  j2202 = C3133 = 0
	   75:  j2210 = C3231 = 0
	   76:  j2211 = C3232 = -1/(2*(alpha**2*deltaT*gammaFTau*s23**2*(n - 1)/(j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s23*s23T*(-alpha + 1)*(n - 1)/(j2FTau*j2FTplusDt) + 1/(2*mu)))
	   77:  j2212 = C3233 = 0
	   78:  j2220 = C3331 = 0
	   79:  j2221 = C3332 = 0
	   80:  j2222 = C3333 = bulkModulus - 2/(3*(alpha**2*deltaT*gammaFTau*s33**2*(n - 1)/(2*j2FTau*j2FTplusDt) + alpha*deltaT*gammaFTau + alpha*deltaT*gammaFTau*s33*s33T*(-alpha + 1)*(n - 1)/(2*j2FTau*j2FTplusDt) + 1/(2*mu)))
	*/
