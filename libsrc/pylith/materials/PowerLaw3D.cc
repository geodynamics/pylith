...
namespace pylith {
	namespace materials {
		namespace _PowerLaw3D{
			...
			/// Physical properties.
			const Metadata::ParamDescription properties[] = {{ "density", 1, pylith::topology::FieldBase::SCALAR },
			...
			const char* dbProperties[] = {"density", "vs", "vp" , "reference-strain-rate", "reference-stress",
										  "power-law-exponent"};
			...
			/// State variables.
			const Metadata::ParamDescription stateVars[] = {{ "viscous_strain", tensorSize, pylith::topology::FieldBase::TENSOR },
...
const int pylith::materials::PowerLaw3D::p_mu = pylith::materials::PowerLaw3D::p_density + 1;

const int pylith::materials::PowerLaw3D::p_lambda = pylith::materials::PowerLaw3D::p_mu + 1;

const int pylith::materials::PowerLaw3D::p_referenceStrainRate = pylith::materials::PowerLaw3D::p_lambda + 1;

const int pylith::materials::PowerLaw3D::p_referenceStress = pylith::materials::PowerLaw3D::p_referenceStrainRate + 1;

const int pylith::materials::PowerLaw3D::p_powerLawExponent = pylith::materials::PowerLaw3D::p_referenceStress + 1;

// Indices of property database values (order must match dbProperties).
const int pylith::materials::PowerLaw3D::db_density = 0;

const int pylith::materials::PowerLaw3D::db_vs = pylith::materials::PowerLaw3D::db_density + 1;

const int pylith::materials::PowerLaw3D::db_vp = pylith::materials::PowerLaw3D::db_vs + 1;

const int pylith::materials::PowerLaw3D::db_referenceStrainRate = pylith::materials::PowerLaw3D::db_vp + 1;

const int pylith::materials::PowerLaw3D::db_referenceStress = pylith::materials::PowerLaw3D::db_referenceStrainRate + 1;

const int pylith::materials::PowerLaw3D::db_powerLawExponent = pylith::materials::PowerLaw3D::db_referenceStress + 1;

// Indices of state variables.
const int pylith::materials::PowerLaw3D::s_viscousStrain = 0;

const int pylith::materials::PowerLaw3D::s_stress = pylith::materials::PowerLaw3D::s_viscousStrain + _PowerLaw3D::tensorSize;
...
const int pylith::materials::PowerLaw3D::db_stress = pylith::materials::PowerLaw3D::db_viscousStrain + _PowerLaw3D::tensorSize;
...
{ // useElasticBehavior
    ...
    _updateStateVarsFn = &pylith::materials::PowerLaw3D::_updateStateVarsElastic;

	} else {
		_calcStressFn = &pylith::materials::PowerLaw3D::_calcStressViscoelastic;
		_calcElasticConstsFn = &pylith::materials::PowerLaw3D::_calcElasticConstsViscoelastic;
		_updateStateVarsFn = &pylith::materials::PowerLaw3D::_updateStateVarsViscoelastic;
	} // if/else
} // useElasticBehavior
...
void
pylith::materials::PowerLaw3D::_dbToProperties(PylithScalar* const propValues, const scalar_array& dbValues)
{ // _dbToProperties
	assert(0 != propValues);
...
void
pylith::materials::PowerLaw3D::_nondimProperties(PylithScalar* const values, const int nvalues) const
{ // _nondimProperties
	...
	values[p_density] = _normalizer->nondimensionalize(values[p_density], densityScale);
	values[p_mu] = _normalizer->nondimensionalize(values[p_mu], pressureScale);
	values[p_lambda] = _normalizer->nondimensionalize(values[p_lambda], pressureScale);
	values[p_referenceStrainRate] = _normalizer->nondimensionalize(values[p_referenceStrainRate], strainRateScale);
	values[p_referenceStress] = _normalizer->nondimensionalize(values[p_referenceStress], pressureScale);
...
void
pylith::materials::PowerLaw3D::_dimProperties(PylithScalar* const values, const int nvalues) const
{ // _dimProperties
	assert(0 != _normalizer);
	assert(0 != values);
	assert(nvalues == _numPropsQuadPt);
	...
	values[p_density] = _normalizer->dimensionalize(values[p_density], densityScale);
	values[p_mu] = _normalizer->dimensionalize(values[p_mu], pressureScale);
	values[p_lambda] = _normalizer->dimensionalize(values[p_lambda], pressureScale);
	values[p_referenceStrainRate] = _normalizer->dimensionalize(values[p_referenceStrainRate], strainRateScale);
	values[p_referenceStress] = _normalizer->dimensionalize(values[p_referenceStress], pressureScale);
...
void
pylith::materials::PowerLaw3D::_dbToStateVars( PylithScalar* const stateValues, const scalar_array& dbValues)
{ // _dbToStateVars
	...
	memcpy(&stateValues[s_viscousStrain], &dbValues[db_viscousStrain], _tensorSize*sizeof(PylithScalar));
	memcpy(&stateValues[s_stress], &dbValues[db_stress], _tensorSize*sizeof(PylithScalar));
...
void
pylith::materials::PowerLaw3D::_nondimStateVars(PylithScalar* const values, const int nvalues) const
{ // _nondimStateVars
	assert(0 != _normalizer);
	assert(0 != values);
...
void
pylith::materials::PowerLaw3D::_dimStateVars(PylithScalar* const values, const int nvalues) const
{ // _dimStateVars
	assert(0 != _normalizer);
	assert(0 != values);
	assert(nvalues == _numVarsQuadPt);
...
PylithScalar
pylith::materials::PowerLaw3D::_stableTimeStepImplicit(const PylithScalar* properties,
													   const int numProperties,
													   const PylithScalar* stateVars,
...
{ // _stableTimeStepImplicit
	...
	if (effStress <= 0.0) {
		...
		dtTest = pow((referenceStress/effStress), (powerLawExp - 1.0)) * (referenceStress/mu)/(referenceStrainRate * 30.0);
	} //else
...
// material.
void
pylith::materials::PowerLaw3D::_calcStressElastic(PylithScalar* const stress,
...
// material.
void
pylith::materials::PowerLaw3D::_calcStressViscoelastic(PylithScalar* const stress,
													   const int stressSize,
													   const PylithScalar* properties,
													   const int numProperties,
													   const PylithScalar* stateVars,
...
{ // _calcStressViscoelastic
	...
	if (computeStateVars) {
		...
		const PylithScalar meanStressInitial = (initialStress[0] + initialStress[1] + initialStress[2])/3.0;
		const PylithScalar devStressInitial[tensorSize] = {initialStress[0] - meanStressInitial,
														   initialStress[1] - meanStressInitial,
														   initialStress[2] - meanStressInitial,
														   initialStress[3],
														   initialStress[4],
														   initialStress[5]};
		const PylithScalar stressInvar2Initial = 0.5 * scalarProduct3D(devStressInitial, devStressInitial);

		// Initial strain values
		const PylithScalar meanStrainInitial = (initialStrain[0] + initialStrain[1] + initialStrain[2])/3.0;
		...
		const PylithScalar strainPPTpdt[tensorSize] = {totalStrain[0] - meanStrainTpdt - visStrainT[0] - initialStrain[0],
		...
		const PylithScalar strainPPInvar2Tpdt = 0.5 * scalarProduct3D(strainPPTpdt, strainPPTpdt);
		...
		const PylithScalar devStressT[tensorSize] = {stressT[0] - meanStressT,
													 ...
													 stressT[4],
													 stressT[5]};
		const PylithScalar stressInvar2T = 0.5 * scalarProduct3D(devStressT, devStressT);
		const PylithScalar effStressT = sqrt(stressInvar2T);

		// Finish defining parameters needed for root-finding algorithm.
		const PylithScalar b = strainPPInvar2Tpdt + ae * scalarProduct3D(strainPPTpdt, devStressInitial) +
			ae * ae * stressInvar2Initial;
		const PylithScalar c = (scalarProduct3D(strainPPTpdt, devStressT) +
		...
		if (b != 0.0 || c != 0.0 || d != 0.0) {
			...
			effStressTpdt = EffectiveStress::calculate<PowerLaw3D>(effStressInitialGuess, stressScale, this);
		} // if

		// Compute stresses from effective stress.
		const PylithScalar effStressTau = (1.0 - alpha) * effStressT + alpha * effStressTpdt;
		const PylithScalar gammaTau = referenceStrainRate * pow((effStressTau/referenceStress),
		...
		for (int iComp=0; iComp < tensorSize; ++iComp) {
			...
			stress[iComp] = devStressTpdt + diag[iComp] * (meanStressTpdt + meanStressInitial);
		} // for
...
{ // effStressFunc
	...
	const PylithScalar gammaTau = referenceStrainRate * pow((effStressTau/referenceStress), (powerLawExp - 1.0))/referenceStress;
	const PylithScalar a = ae + alpha * dt * gammaTau;
	const PylithScalar y = a * a * effStressTpdt * effStressTpdt - b + c * gammaTau - d * d * gammaTau * gammaTau;
...
{ // effStressDFunc
	...
	const PylithScalar dGammaTau = referenceStrainRate * alpha * (powerLawExp - 1.0) *
		pow((effStressTau/referenceStress), (powerLawExp - 2.0))/(referenceStress * referenceStress);
	const PylithScalar dy = 2.0 * a * a * effStressTpdt + dGammaTau *
		(2.0 * a * alpha * dt * effStressTpdt * effStressTpdt + c - 2.0 * d * d * gammaTau);
	PetscLogFlops(36);
...
// and derivative.
void
pylith::materials::PowerLaw3D::effStressFuncDerivFunc(PylithScalar* func, PylithScalar* dfunc, const PylithScalar effStressTpdt)
{ // effStressFuncDFunc
	PylithScalar y = *func;
	...
	const PylithScalar dGammaTau = referenceStrainRate * alpha * (powerLawExp - 1.0) *
		pow((effStressTau/referenceStress), (powerLawExp - 2.0))/(referenceStress * referenceStress);
	const PylithScalar a = ae + alpha * dt * gammaTau;
	y = a * a * effStressTpdt * effStressTpdt - b + c * gammaTau - d * d * gammaTau * gammaTau;
	dy = 2.0 * a * a * effStressTpdt + dGammaTau *
		(2.0 * a * alpha * dt * effStressTpdt * effStressTpdt + c - 2.0 * d * d * gammaTau);
...
void
pylith::materials::PowerLaw3D::_calcElasticConstsElastic(PylithScalar* const elasticConsts,
...
void
pylith::materials::PowerLaw3D::_calcElasticConstsViscoelastic(PylithScalar* const elasticConsts,
															  const int numElasticConsts,
															  const PylithScalar* properties,
															  const int numProperties,
															  const PylithScalar* stateVars,
...
{ // _calcElasticConstsViscoelastic
	...
	if (b == 0.0 && c == 0.0 && d == 0.0) {
		...
		const PylithScalar effStressTpdt = EffectiveStress::calculate<PowerLaw3D>(effStressInitialGuess, stressScale, this);
		...
		const PylithScalar devStressTpdt[tensorSize] = {
			factor1*(strainPPTpdt[0] - factor2*devStressT[0] + ae*devStressInitial[0]),
			factor1*(strainPPTpdt[1] - factor2*devStressT[1] + ae*devStressInitial[1]),
			factor1*(strainPPTpdt[2] - factor2*devStressT[2] + ae*devStressInitial[2]),
			factor1*(strainPPTpdt[3] - factor2*devStressT[3] + ae*devStressInitial[3]),
			factor1*(strainPPTpdt[4] - factor2*devStressT[4] + ae*devStressInitial[4]),
			factor1*(strainPPTpdt[5] - factor2*devStressT[5] + ae*devStressInitial[5])
		};
		const PylithScalar devStressTau[tensorSize] = {alpha * devStressTpdt[0] + explicitFac * devStressT[0],
													   alpha * devStressTpdt[1] + explicitFac * devStressT[1],
													   alpha * devStressTpdt[2] + explicitFac * devStressT[2],
													   alpha * devStressTpdt[3] + explicitFac * devStressT[3],
													   alpha * devStressTpdt[4] + explicitFac * devStressT[4],
													   alpha * devStressTpdt[5] + explicitFac * devStressT[5]};
#if 1 // Comment out old method.
		const PylithScalar factor3 = 0.5 * referenceStrainRate * _dt * alpha * (powerLawExp - 1.0) *
			pow((effStressTau/referenceStress), (powerLawExp - 2.0))/(referenceStress * referenceStress * effStressTpdt);

		// Compute deviatoric derivatives
		const PylithScalar dStress11dStrain11 = 1.0/(a + devStressTau[0] * devStressTpdt[0] * factor3);
		const PylithScalar dStress22dStrain22 = 1.0/(a + devStressTau[1] * devStressTpdt[1] * factor3);
		const PylithScalar dStress33dStrain33 = 1.0/(a + devStressTau[2] * devStressTpdt[2] * factor3);
		const PylithScalar dStress12dStrain12 = 1.0/(a + 2.0 * devStressTau[3] * devStressTpdt[3] * factor3);
		const PylithScalar dStress23dStrain23 = 1.0/(a + 2.0 * devStressTau[4] * devStressTpdt[4] * factor3);
		const PylithScalar dStress13dStrain13 = 1.0/(a + 2.0 * devStressTau[5] * devStressTpdt[5] * factor3);
		...
		PetscLogFlops(114);
#endif // End comment out old method.

#if 0 // Comment out new method.
		const PylithScalar j2Mult = effStressTau*effStressTau;
		const PylithScalar f3 = _dt*gammaTau;
		const PylithScalar f4 = 0.5*f3*(powerLawExp - 1.0);
		const PylithScalar f5 = f3 + ae;
    
		/// Compute tangent matrix.
		// Form elastic constants.
		elasticConsts[ 0] = bulkModulus + 2.0/(3.0*(f5 + f4*pow(devStressTau[0]/effStressTau, 2.0))); // C1111
		elasticConsts[ 1] = bulkModulus - 1.0/(3.0*(f5 + f4*pow(devStressTau[0]/effStressTau, 2.0))); // C1122
		elasticConsts[ 2] = bulkModulus - 1.0/(3.0*(f5 + f4*pow(devStressTau[0]/effStressTau, 2.0))); // C1133
		elasticConsts[ 3] = 0.0; // C1112
		elasticConsts[ 4] = 0.0; // C1123
		elasticConsts[ 5] = 0.0; // C1113
		elasticConsts[ 6] = elasticConsts[ 1]; // C2211
		elasticConsts[ 7] = bulkModulus + 2.0/(3.0*(f5 + f4*pow(devStressTau[1]/effStressTau, 2.0))); // C2222
		elasticConsts[ 8] = bulkModulus - 1.0/(3.0*(f5 + f4*pow(devStressTau[1]/effStressTau, 2.0))); // C2233
		elasticConsts[ 9] = 0.0; // C2212
		elasticConsts[10] = 0.0; // C2223
		elasticConsts[11] = 0.0; // C2213
		elasticConsts[12] = elasticConsts[ 2]; // C3311
		elasticConsts[13] = elasticConsts[ 8]; // C3322
		elasticConsts[14] = bulkModulus + 2.0/(3.0*(f5 + f4*pow(devStressTau[2]/effStressTau, 2.0))); // C3333
		elasticConsts[15] = 0; // C3312
		elasticConsts[16] = 0; // C3323
		elasticConsts[17] = 0; // C3313
		elasticConsts[18] = 0; // C1211
		elasticConsts[19] = 0; // C1222
		elasticConsts[20] = 0; // C1233
		elasticConsts[21] = 1.0/(2.0*(f5 + 2.0*f4*pow(devStressTau[3]/effStressTau, 2.0))); // C1212
		elasticConsts[22] = 0; // C1223
		elasticConsts[23] = 0; // C1213
		elasticConsts[24] = 0; // C2311
		elasticConsts[25] = 0; // C2322
		elasticConsts[26] = 0; // C2333
		elasticConsts[27] = 0; // C2312
		elasticConsts[28] = 1.0/(2.0*(f5 + 2.0*f4*pow(devStressTau[4]*effStressTau, 2.0))); // C2323
		elasticConsts[29] = 0; // C2313
		elasticConsts[30] = 0; // C1311
		elasticConsts[31] = 0; // C1322
		elasticConsts[32] = 0; // C1333
		elasticConsts[33] = 0; // C1312
		elasticConsts[34] = 0; // C1323
		elasticConsts[35] = 1.0/(2.0*(f5 + 2.0*f4*pow(devStressTau[5]*effStressTau, 2.0))); // C1313
		PetscLogFlops(130);
#endif // End comment out new method.
	} // else
} // _calcElasticConstsViscoelastic
...
void
pylith::materials::PowerLaw3D::_updateStateVarsElastic(PylithScalar* const stateVars,
													   const int numStateVars,
													   const PylithScalar* properties,
													   const int numProperties,
													   const PylithScalar* totalStrain,
...
// Update state variables.
void
pylith::materials::PowerLaw3D::_updateStateVarsViscoelastic(PylithScalar* const stateVars,
															const int numStateVars,
															const PylithScalar* properties,
															const int numProperties,
															const PylithScalar* totalStrain,
...
{ // _updateStateVarsViscoelastic
	...
	for (int iComp=0; iComp < tensorSize; ++iComp) {
		devStressTpdt = factor1 * (strainPPTpdt[iComp] - factor2 * devStressT[iComp] + ae * devStressInitial[iComp]);
		stateVars[s_stress+iComp] = devStressTpdt + diag[iComp] * (meanStressTpdt + meanStressInitial);
