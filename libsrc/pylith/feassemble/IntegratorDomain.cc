...
pylith::feassemble::IntegratorDomain::computeRHSResidual(pylith::topology::Field* residual,
    ...
    err = DMPlexComputeResidual_Internal(dsLabel.dm(), key, dsLabel.cellsIS(), PETSC_MIN_REAL, solution.localVector(),
                                         solutionDotVec, t, residual->localVector(), NULL);PYLITH_CHECK_ERROR(err);
...
pylith::feassemble::IntegratorDomain::computeLHSResidual(pylith::topology::Field* residual,
    ...
    assert(residual->localVector());
    err = DMPlexComputeResidual_Internal(dsLabel.dm(), key, dsLabel.cellsIS(), PETSC_MIN_REAL, solution.localVector(),
                                         solutionDot.localVector(), t, residual->localVector(), NULL);PYLITH_CHECK_ERROR(err);
...
pylith::feassemble::IntegratorDomain::computeLHSJacobian(PetscMat jacobianMat,
    ...
    assert(jacobianMat);
    assert(precondMat);
    err = DMPlexComputeJacobian_Internal(dsLabel.dm(), key, dsLabel.cellsIS(), t, s_tshift, solution.localVector(),
...
pylith::feassemble::IntegratorDomain::computeLHSJacobianLumpedInv(pylith::topology::Field* jacobianInv,
    ...
    err = DMPlexComputeJacobian_Action_Internal(dsLabel.dm(), key, dsLabel.cellsIS(), t, s_tshift, vecRowSum, NULL,
                                                vecRowSum, jacobianInv->localVector(), NULL);PYLITH_CHECK_ERROR(err);
...
pylith::feassemble::IntegratorDomain::_updateStateVars(const PylithReal t,
    ...
    err = DMSetAuxiliaryVec(stateVarsDM, dmLabel, labelValue, _auxiliaryField->localVector());PYLITH_CHECK_ERROR(err);
