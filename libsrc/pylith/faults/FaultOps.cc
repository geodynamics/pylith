...
void
pylith::faults::FaultOps::createDAEMassWeighting(pylith::problems::IntegrationData* integrationData,
                                                 const pylith::topology::Field& solution) {
    PYLITH_METHOD_BEGIN;
    typedef pylith::problems::SolutionFactory SolutionFactory;

    const pylith::topology::Field::SubfieldInfo& velocityInfo = solution.getSubfieldInfo(SolutionFactory::velocity.c_str());
    const pylith::topology::Field::SubfieldInfo& lagrangeInfo = solution.getSubfieldInfo(SolutionFactory::lagrange_multiplier_fault.c_str());
    ...
    const char* dae_mass_weighting = pylith::problems::IntegrationData::dae_mass_weighting.c_str();
...
// Create weighting vector for dynamic prescribed slip DAE.
void
pylith::faults::FaultOps::updateDAEMassWeighting(pylith::problems::IntegrationData* integrationData) {
    PYLITH_METHOD_BEGIN;
    PetscErrorCode err;

    const pylith::topology::Field* jacobianLumpedInv =
        integrationData->getField(pylith::problems::IntegrationData::lumped_jacobian_inverse);assert(jacobianLumpedInv);
    const pylith::topology::Field* weighting =
        integrationData->getField(pylith::problems::IntegrationData::dae_mass_weighting);assert(jacobianLumpedInv);

    const char* velocity = pylith::problems::SolutionFactory::velocity.c_str();
