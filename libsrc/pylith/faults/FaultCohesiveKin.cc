...
pylith::faults::FaultCohesiveKin::deallocate(void) {
    ...
    PylithCallPetsc(VecDestroy(&_slipVecRupture));
    PylithCallPetsc(VecDestroy(&_slipVecTotal));
...
pylith::faults::FaultCohesiveKin::createAuxiliaryField(const pylith::topology::Field& solution,
    ...
    PylithCallPetsc(DMCreateLocalVector(auxiliaryField->getDM(), &_slipVecRupture));
    PylithCallPetsc(DMCreateLocalVector(auxiliaryField->getDM(), &_slipVecTotal));
...
pylith::faults::FaultCohesiveKin::_updateSlip(pylith::topology::Field* auxiliaryField,
    ...
    PylithCallPetsc(VecSet(_slipVecTotal, 0.0));
    const srcs_type::const_iterator rupturesEnd = _ruptures.end();
    for (srcs_type::iterator r_iter = _ruptures.begin(); r_iter != rupturesEnd; ++r_iter) {
        PylithCallPetsc(VecSet(_slipVecRupture, 0.0));
        ...
        PylithCallPetsc(VecAYPX(_slipVecTotal, 1.0, _slipVecRupture));
    } // for

    // Transfer slip values from local PETSc slip vector to fault auxiliary field.
    PetscInt pStart = 0, pEnd = 0;
    PylithCallPetsc(PetscSectionGetChart(auxiliaryField->getLocalSection(), &pStart, &pEnd));
    ...
    PylithCallPetsc(VecGetArrayRead(_slipVecTotal, &slipArray));
...
pylith::faults::FaultCohesiveKin::_setKernelsResidual(pylith::feassemble::IntegratorInterface* integrator,
    ...
    case pylith::problems::Physics::DYNAMIC_IMEX: {
        // Elasticity equation (displacement) for negative side of the fault.
        PetscBdPointFn* g0v_neg = pylith::fekernels::FaultCohesiveKin::f0u_neg;
        PetscBdPointFn* g1v_neg = NULL;

        // Elasticity equation (displacement) for positive side of the fault.
        PetscBdPointFn* g0v_pos = pylith::fekernels::FaultCohesiveKin::f0u_pos;
        PetscBdPointFn* g1v_pos = NULL;

        // Fault slip constraint equation.
        PetscBdPointFn* f0l_slip = pylith::fekernels::FaultCohesiveKin::f0l_slip;
        PetscBdPointFn* f1l_slip = NULL;

        // Fault DAE equation.
        PetscBdPointFn* f0l_dae = pylith::fekernels::FaultCohesiveKin::f0l_slipAcc;
        PetscBdPointFn* f1l_dae = NULL;
