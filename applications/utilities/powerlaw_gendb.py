...
class PowerLawApp(Application):
    ...
    # @li \b database_filename Object for writing database.
    ...
    refStrainRate = pyre.inventory.dimensional("reference_strain_rate", default=1.0e-6 / s)
    refStrainRate.meta['tip'] = "Reference strain rate value."

    from spatialdata.spatialdb.SimpleDB import SimpleDB

    dbExponent = pyre.inventory.facility("db_exponent", family="spatial_database", factory=SimpleDB)
    dbExponent.meta['tip'] = "Spatial db for power-law exponent, n."

    dbActivationE = pyre.inventory.facility("db_activation_energy", family="spatial_database", factory=SimpleDB)
    dbActivationE.meta['tip'] = "Spatial db for activation energy, Q."

    dbTemperature = pyre.inventory.facility("db_temperature", family="spatial_database", factory=SimpleDB)
    dbTemperature.meta['tip'] = "Spatial db for temperature, T."

    dbAe = pyre.inventory.facility("db_powerlaw_coefficient", family="spatial_database", factory=SimpleDB)
    ...
    dbFilename = pyre.inventory.str("database_filename", default="powerlaw.spatialdb")
    ...
    def main(self, *args, **kwds):
        ...
        if self.refSelection == "stress":
            ...
            raise ValueError("Invalid value (%s) for reference value." % self.refSelection)
