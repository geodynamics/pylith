...
The function `create_material()` in the `gmsh_utils` module automatically generates physical groups following this convention.
...
The physical groups must include entities at the topological dimension of the boundary condition as well as all lower dimensions.
...
The `gmsh_utils` module also includes a template application object (Python abstract base class) for writing Python application scripts for generating meshes using Gmsh.
The template application defines the steps for generating the mesh with a separate function (to be implemented by the user) for each step.
