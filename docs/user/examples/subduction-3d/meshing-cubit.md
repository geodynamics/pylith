...
We provide the Python script `viz/plot_mesh.py` to visualize the nodesets and the mesh quality using the condition number metric. If you have `pvpython` in your `$PATH`, you can simply run:
...
To visualize the mesh using the ParaView GUI Python shell, start ParaView.
Within the ParaView GUI Python shell (`View`$\rightarrow$`Python Shell`), we override the `EXODUS_FILE` and `SHOW_QUALITY` parameters.
