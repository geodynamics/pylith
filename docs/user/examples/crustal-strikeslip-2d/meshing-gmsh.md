...
{numref}`fig:example:crustal:strikeslip:2d:geometry:gmsh` shows the geometry and variables names of the vertices and curves.

:::{figure-md} fig:example:crustal:strikeslip:2d:geometry:gmsh
...
We construct curves from points (`pSW`, ..., `pNW`) and surfaces from the curves (for example, `c_north`).
...
:::{important}
In order for Gmsh to create cells with edges that align with the fault traces, we must embed the curves into the surface.

Additionally, we must split the main fault trace into two so that there is a common point among all three fault traces where they intersect.
:::
...
:::{figure-md} fig:example:crustal:strikeslip:2d:gmsh:mesh
