...
2. Run just the residual test for a single discretization and use the debug journal command line argument to the MMS test driver (`--journal.debug=mms-test`).
...
2. Run just the Jacobian finite-difference test for a single discretization and use the debug journal command line argument to the MMS test drive (`--journal.debug=mms-test`).
