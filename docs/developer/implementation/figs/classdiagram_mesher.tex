...
\begin{tikzpicture}[node distance=10em, line width=1.5pt]
  ...
  \node (petsc-component) [abstract-class] at ($(pythia-component.south)-(0,4em)$) {\umlemptyclass{Component}};
  ...
  \node (meshio-petsc) [concrete-class] at ($(meshio-cubit.south)-(0,1ex)$) {\umlemptyclass{MeshIOPetsc}};

  \node (distributor) [concrete-class] at ($(meshio-petsc.south)-(0,3em)$) {\umlemptyclass{Distributor}};
