...
\begin{tikzpicture}[node distance=18.0em, line width=1.5pt]
  ...
  \draw (upstream-main.south) -- ++(0,-18em);
  \path let \p1=(upstream-main.south) in coordinate (x-upstream-main) at (\x1,0);

  \node (origin-branch) [branch] at ($(developer-github.south)+(0,-4mm)$) {feature-powerlaw};
  \draw ($(origin-branch.south)-(0,4em)$) -- ++(0,-9em);
  ...
  \draw ($(local-branch.south)-(0,2em)$) -- ++(0,-11em);
  ...
  \path[arrow-down] ($(x-upstream-main)+(currenty)$) edge node[above, pos=0.8] {fetch} ($(x-local-main)+(currenty)$);
  ...
  \path[arrow-down] ($(x-upstream-main)+(currenty)$) edge node[above, pos=0.9] {fetch} ($(x-local-main)+(currenty)$);
